...
export async function getEvents(
  ...
  try {
    ...
    if (!sheetId) {
      return { error: "Invalid Google Sheet URL format. Could not find sheet ID." };
    }
    ...
    const jsonString = responseText.match(/(?<=google\.visualization\.Query\.setResponse\().*(?=\);)/s)?.[0];
    ...
    const requiredHeaders = ["title", "subtitle", "description", "date"];
      ...
      row.c.forEach((cell, i) => {
        ...
        if (header) {
          if (cell) {
            ...
            if(header === 'date' && typeof cell.v === 'string' && cell.v.startsWith('Date(')){
              const dateParts = cell.v.replace('Date(', '').replace(')', '').split(',');
              const dateObj = new Date(parseInt(dateParts[0]), parseInt(dateParts[1]), parseInt(dateParts[2]));
